...
Se quiere caracterizar aspectos fundamentales de la **estructura social** de una red representada con un grafo $G=(V,E)$:
...
knitr::include_graphics("05_got_degree_top10.png")
```
...
knitr::include_graphics("05_got_strength_top10.png")
```
...
plot(got, 
     layout = l, 
     vertex.size = 1.5*sqrt(d), 
     vertex.label = NA, 
     vertex.color = adjustcolor("royalblue",0.2), 
     vertex.frame.color = "royalblue", 
     edge.color = adjustcolor("gray",0.4))
title(sub = "Grado", line = -1)

# usando la fuerza
plot(got, 
     layout = l, 
     vertex.size = 0.3*sqrt(wd), 
     vertex.label = NA, 
     vertex.color = adjustcolor("royalblue",0.2), 
     vertex.frame.color = "royalblue",
     edge.color = adjustcolor("gray",0.4))
...
# distribución del grado log-log
plot(x = (0:max(d))[dd != 0], y = dd[dd != 0], 
     log = "xy", 
     pch = 16, 
     col = adjustcolor("royalblue", 0.5), 
     xlab = "Log-grado", 
     ylab = "Log-densidad", 
...
plot(x = d, 
     ...
     log = "xy", 
     pch = 16, 
     col = adjustcolor("yellow3", 0.5), 
     xlab = "Log-grado", 
     ylab = "Log-grado promedio de los vecinos")
```
...
```{r}
# betweenness centrality no normalizada
...
head(
  cbind(
    eigen_centrality(graph = karate)$vector,
    eigen_centrality(graph = g)$vector,
    Y%*%c(1/evd$values[1]*(-1)*evd$vectors[,1])/max(Y%*%c(1/evd$values[1]*(-1)*evd$vectors[,1]))), 
  n = 5)
# top 5
ec <- eigen_centrality(graph = karate)$vector
...
hits <- igraph::hits_scores(graph = aidsblog, scale = TRUE)

hs <- hits$hub        # hub scores
as <- hits$authority  # authority scores
```
...
Otras medidas populares de centralidad de nodos son:
