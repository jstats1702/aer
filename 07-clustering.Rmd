...
knitr::include_graphics("07_partition.png")
```
...
```{r, eval = TRUE, echo=FALSE, out.width="40%", fig.pos = 'H', fig.align = 'center'}
knitr::include_graphics("07_clustering_ex.png")
```
...
knitr::include_graphics("07_clustering_agg_div.png")
```
...
knitr::include_graphics("07_clustering_mod.png")
```
...
Si \(\mathbf{Y} = [y_{i,j}]\) es la matriz de adyacencia de un grafo no dirigido, la matriz laplaciana se define como
\[
\mathbf{L} = \mathbf{D} - \mathbf{Y},
\]
donde \(\mathbf{D} = \textsf{diag}(d_1,\ldots,d_n)\) es la matriz diagonal de grados, con entradas
\[
d_i = \sum_j y_{i,j}.
\]
...
Por otra parte, también es común utilizar la **matriz de modularidad**. Si \(\mathbf{Y}\) es la matriz de adyacencia del grafo, la matriz de modularidad se define como \(\mathbf{B} = [b_{i,j}]\), donde
\[
b_{i,j} = y_{i,j} - \frac{1}{2m}\,d_i d_j,
\]
y \(m\) denota el número total de aristas del grafo. 
...
knitr::include_graphics("07_clustering_spectral.png")
```
